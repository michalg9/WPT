...
for i = 1:1
    ...
    for j = 1:length(rxEventLog)
        ...
        if(strcmp(rxEventLog{j,2}, 'event') && rxEventLog{j,3} == 2)
            tsRxAbove = [tsRxAbove, rxEventLog{j,1}];
        end
        
        if(strcmp(rxEventLog{j,2}, 'event') && rxEventLog{j,3} == 6)
            tsRxBelow = [tsRxBelow, rxEventLog{j,1}];
        end
    end
